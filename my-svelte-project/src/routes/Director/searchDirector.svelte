  ...
  <main>
    ...
    <ul>
      {#each filteredDirectors as director}
        <li>
          <button on:click={() => selectDirector(director)}>{director.firstName}{director.lastName}</button>
        </li>
      {/each}
    </ul>
  
    {#if selectedDirector}
      <h2>Movies by {selectedDirector.firstName}{selectedDirector.lastName}</h2>
      <ul>
